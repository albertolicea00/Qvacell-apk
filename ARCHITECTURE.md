...
| `service/DialService.kt`                           | `dial()` builds/opens `tel:` URIs via `ACTION_DIAL` (used everywhere — Ayuda, SMS services, Contactos, Transferir/Recargar); `dialDirect()` uses `ACTION_CALL` instead, used only by Compras' purchase confirmation flow once `CALL_PHONE` is granted. Also SMS intents via `ACTION_SENDTO` and Google Maps search URLs via `ACTION_VIEW`.                        |
