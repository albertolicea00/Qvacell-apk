...
Qvacell ships as a single, continuously updated Android app (no parallel major-version branches). Only the latest release / latest commit on `main` receives security fixes.
...
- The USSD catalog (`app/src/main/assets/codes.json`) is a static file bundled at build time.
- The only privileged action the app performs is handing a dial string to Android via `ACTION_DIAL` — it does not place calls itself, and holds no `CALL_PHONE` permission.
- The `CallScreeningService` (§11 in [ARCHITECTURE.md](ARCHITECTURE.md)) never blocks or rejects calls — it only reads the incoming number to unwrap ETECSA's `*99` collect-call format.
- No analytics, no ads SDKs, no third-party dependencies beyond AndroidX/Jetpack (per [CONTRIBUTING.md](CONTRIBUTING.md): "No third-party dependencies beyond AndroidX/Jetpack unless discussed in an issue first").

Given that, realistic security concerns for this project are narrower than a typical networked app. In scope:

- **Malicious or malformed USSD codes** in `codes.json` that could dial premium-rate numbers, trigger unintended carrier actions, or crash the app via bad JSON/URI-encoding.
...
- **Local data exposure** — the transfer PIN store (`TransferPinStore`, EncryptedSharedPreferences) or the imported directory `.db` file being readable by another app due to incorrect storage scope or file permissions.
- **Build tooling issues** (Gradle scripts, CI workflows in `.github/`) that could lead to unintended code execution during build/CI.
...
1. **Preferred:** [GitHub Security Advisories](https://github.com/albertolicea00/qvacell-apk/security/advisories/new) for this repository — lets the maintainer triage privately and coordinate a fix/disclosure.
...
- Steps to reproduce (device/Android version, the specific USSD entry or input value involved).
- The affected file(s) (e.g. `app/src/main/assets/codes.json`, a specific service class, a specific CI workflow).
